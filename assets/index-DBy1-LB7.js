...
const makePaginationKeyboardAccessible = (swiper) => {
...
const addNavigationIcons = (swiper, prevIcon = "arrow-left", nextIcon = "arrow-right") => {
  const prevBtn = swiper.navigation.prevEl;
  const nextBtn = swiper.navigation.nextEl;
  if (prevBtn) {
    prevBtn.innerHTML = `
      <svg aria-hidden="true">
        <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-${prevIcon}"></use>
      </svg>
    `;
  }
  if (nextBtn) {
    nextBtn.innerHTML = `
      <svg aria-hidden="true">
        <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-${nextIcon}"></use>
      </svg>
    `;
  }
};
...
const initSliders = () => {
  ...
  void new Swiper('[data-slider="training"]', {
    ...
    slidesPerView: 1,
    initialSlide: 2,
    // Mobile: starts from 3rd slide (Nadezhda)
    spaceBetween: 20,
    speed: 400,
    resistance: true,
    resistanceRatio: 0.85,
    breakpoints: {
      768: { slidesPerView: 3, initialSlide: 0 },
      // Tablet/Desktop: from 1st slide
...
const validateAllInputs = (form, options = {}) => {
  const {
...
const TIMING = {
  ...
  MESSAGE_DISPLAY: 5e3,
  // How long to show success/error messages
  MODAL_CLOSE_DELAY: 1e3,
  // Delay before closing modal after success
  SCROLL_DELAY: 100,
  // Delay before scrolling to message
  FORM_CLEAR_DELAY: 500
  // Delay before clearing form after success
};
  ...
  function createMobileLines(processItems) {
    ...
    processItems.forEach((item, index) => {
      if (index === processItems.length - 1) {
        ...
        line.style.height = `${circleY - (itemRect.top - containerRect.top) - 7}px`;
        ...
        line.style.left = "6px";
        line.style.top = `${currentCircleY + 7}px`;
        line.style.height = `${nextCircleY - currentCircleY - 14}px`;
        linesContainer.appendChild(line);
      }
    });
  ...
  for (let i = 0; i < 5; i++) {
    ...
    starsHTML += `
      <svg class="icon" width="16" height="16" aria-hidden="true">
        <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-${iconType}"></use>
      </svg>
    `;
  }
  return starsHTML;
    ...
    <div class="card__content">
      <div class="card__meta">
        <span class="card__meta-location">
          <svg class="icon" width="9" height="12" aria-hidden="true">
            <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-pin-fill"></use>
          </svg>
          ${tour.region}
        </span>
        <div class="card__meta-duration-wrapper">
          <span class="card__meta-duration">
            <svg class="icon" width="20" height="20" aria-hidden="true">
              <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-sun"></use>
            </svg>
            ${getDaysText(tour.duration.days)}
          </span>
          <span class="card__meta-duration">
            <svg class="icon" width="16" height="16" aria-hidden="true">
              <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-moon"></use>
            </svg>
      ...
      <button class="button tours-catalog__card-button" type="button" data-tour-detail="${tour.id}">
        ...
        <svg class="icon" width="6" height="12" aria-hidden="true">
          <use href="/travel-landing-swiper-responsive/assets/spritemap.87b4fd38.svg#sprite-arrow-small-right"></use>
        </svg>
      </button>
    </div>
  </article>
...
const filterTours = (filter) => {
  const toursData2 = getToursData();
  if (filter === "all") {
    return toursData2;
  }
  return toursData2.filter((tour) => {
...
const populateTourDetail = (tour) => {
  ...
  if (programElement) {
    programElement.innerHTML = tour.program.map(
      (day) => `
        <article>
          <h4>День ${day.day}</h4>
...
const openTourDetail = (tourId) => {
  const toursData2 = getToursData();
  const tour = toursData2.find((t) => t.id === tourId);
  if (!tour) {
...
const observerOptions = {
  ...
  // Trigger 20px before element enters viewport (reduced for bottom elements)
  threshold: 0.1
};
...
//# sourceMappingURL=index-DBy1-LB7.js.map
